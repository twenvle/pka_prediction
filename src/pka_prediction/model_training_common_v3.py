...
def _build_hyperparameter_search(
    ...
    search = deps["RandomizedSearchCV"](
        estimator=pipeline,
    ...
    return search, "RandomizedSearchCV", parameters
...
def _search_candidate_results(search: Any) -> list[dict[str, Any]]:
    """最終ハイパーパラメータ探索の候補別結果を順位順で返す。"""
    cv_results = search.cv_results_
    candidates = [
        {
            "rank": int(rank),
            "mean_cv_rmse": float(-mean_score),
            "std_cv_rmse": float(std_score),
...
def build_parser(model_name: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="ハイパーパラメータ探索内のK-Fold分割数（既定: 3）",
    )
    parser.add_argument(
        "--search-iterations",
        type=int,
        default=20,
        help="ランダム探索の候補数（既定: 20、Ridgeでは未使用）",
    )
    parser.add_argument(
...
def run_training(model_name: str, argv: Sequence[str] | None = None) -> int:
    ...
    if args.inner_splits < 2:
        parser.error("--inner-splits は2以上にしてください")
    if args.search_iterations < 1:
        parser.error("--search-iterations は1以上にしてください")
    ...
    smallest_outer_train_size = len(X) - math.ceil(len(X) / args.n_splits)
    if smallest_outer_train_size < args.inner_splits:
        parser.error("外側foldの学習行数より --inner-splits が大きいため探索できません")
        ...
        fold_results.append(
            {
                ...
                "inner_cv_best_rmse": float(-fold_search.best_score_),
    ...
    # 保存用モデルは交差検証後に全データで再探索し、最良モデルを保存する。
    ...
    metrics = {
        ...
        "metrics_note": (
            f"train_metricsは外側{args.n_splits}foldの学習指標平均、"
            "test_metricsは外側foldの"
            "検証指標平均です。各外側foldの学習データ内だけでハイパーパラメータ"
            "探索を行っています。標準偏差とfold別結果はcross_validationにあります。"
        ),
        ...
        "hyperparameter_search": {
            ...
            "scoring": "neg_root_mean_squared_error",
            "search_iterations": (
                None if model_name == "ridge" else args.search_iterations
            ),
            "search_space": _json_value(search_space),
            "final_best_parameters": _json_value(final_search.best_params_),
            "final_inner_cv_best_rmse": float(-final_search.best_score_),
    ...
    print(
        "最良パラメータ: "
        + json.dumps(final_search.best_params_, ensure_ascii=False, sort_keys=True)
    )
    print(
        f"Nested {args.n_splits}-Fold CV評価: "
