...
使用例（pandasとRDKitが必要）:
    ...
    from features_v2 import extract_features, features_dataframe
    row = extract_features("CC(=O)O")
    output, exclusion_counts = features_dataframe(df, smiles_column="smiles")

    from features_v2 import FeaturesConfig, features_csv
    result = features_csv(FeaturesConfig("molecules.csv", "result.csv"))
    result.dataframe

features_csvはヘッダー付きのカンマ区切りCSVをpandasで読み書きする。
...
__all__ = [
    "FeaturesConfig", "FeaturesResult", "FEATURE_COLUMNS", "EXCLUSION_REASONS",
    "extract_features", "features_dataframe", "features_csv",
]
...
FEATURE_COLUMNS = (
    "canonical_smiles", "MW", "logP", "TPSA", "HBA", "HBD",
    "rotatable_bonds", "aromatic_rings", "FractionCSP3", "formal_charge",
    "acid_type", "acid_group_count", "acid_center_aromatic_flag",
    "local_heteroatom_count", "local_halogen_count", "nearest_heteroatom_distance",
    "local_conjugation", "local_formal_charge", "error",
)
SITE_COLUMNS = (
    "acid_center_aromatic_flag", "local_heteroatom_count", "local_halogen_count",
    "nearest_heteroatom_distance", "local_conjugation", "local_formal_charge",
)
...
class FeaturesConfig:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "smiles_column", _validate_smiles_column(self.smiles_column))
        ...
        if _same_file(self.input_path, self.output_path):
            raise ValueError("入力ファイルと出力ファイルには異なるパスを指定してください")
...
def _compile_queries(Chem: Any) -> tuple[tuple[int, Any, int], ...]:
    ...
    for acid_type, smarts in ACID_SMARTS:
        ...
        mapped = [atom.GetIdx() for atom in query.GetAtoms() if atom.GetAtomMapNum() == 1]
        if len(mapped) != 1:
...
def _import_dependencies() -> dict[str, Any]:
    ...
    return {
        "pd": pd, "Chem": Chem, "Descriptors": Descriptors, "Crippen": Crippen,
...
def _find_acid_sites(
    mol: Any, queries: Sequence[tuple[int, Any, int]],
) -> list[tuple[int, int]]:
    sites: dict[int, int] = {}
...
def _molecule_features(
    mol: Any, Chem: Any, Descriptors: Any, Crippen: Any, rdMolDescriptors: Any,
) -> dict[str, int | float]:
    return {
...
def _site_features(mol: Any, center_idx: int) -> dict[str, int | str]:
    ...
    local_heteroatom_count = sum(
        ...
        for idx in local_indices if idx != center_idx
    )
    ...
    other_hetero_distances = [
        distance for idx, distance in enumerate(distances)
        if idx != center_idx and distance is not None
...
def extract_features(
    smiles: object, *, dependencies: Mapping[str, Any] | None = None,
) -> dict[str, object]:
    ...
    result.update({
        ...
        **_molecule_features(
            mol, Chem, deps["Descriptors"], deps["Crippen"], deps["rdMolDescriptors"],
        ),
    })
    ...
    result.update({
        ...
        **{
            column: sum(
                int(site[column]) if site[column] != "" else 0 for site in site_features
            )
...
def _resolve_column(columns: Sequence[str], requested: str) -> str:
    ...
    matches = [column for column in columns if column.strip().casefold() == requested.casefold()]
    if not matches:
...
def features_dataframe(
    ...
    *,
    ...
    positions: list[int] = []
    exclusion_counts: Counter[str] = Counter({reason: 0 for reason in EXCLUSION_REASONS})
    ...
    for position, value in enumerate(frame[actual_smiles_column].tolist()):
        features = extract_features(value, dependencies=deps)
        reason = str(features["error"])
        if reason:
            if errors == "raise":
                raise ValueError(f"行{position + 1}（index={frame.index[position]!r}）: {reason}")
            if errors == "drop":
                exclusion_reason = "invalid_smiles" if reason == "Invalid SMILES" else "missing_smiles"
    ...
    for column in FEATURE_COLUMNS:
        dtype = "string" if column in _TEXT_COLUMNS else (
            "Float64" if column in _FLOAT_COLUMNS else "Int64"
        )
        output[column] = feature_frame[column].astype(dtype).array
    return output, dict(exclusion_counts)


def features_csv(config: FeaturesConfig | Mapping[str, Any]) -> FeaturesResult:
    """CSVを読み込み、特徴量付きの新しいCSVを保存する。"""
    if not isinstance(config, FeaturesConfig):
        if not isinstance(config, Mapping):
            raise TypeError("configにはFeaturesConfigまたはMappingを渡してください")
        config = FeaturesConfig(**dict(config))
    ...
    output, exclusion_counts = features_dataframe(
        frame, smiles_column=config.smiles_column, errors=config.errors, dependencies=deps,
    )
    ...
    return FeaturesResult(
        config=config, dataframe=output, rows_read=len(frame), rows_written=len(output),
