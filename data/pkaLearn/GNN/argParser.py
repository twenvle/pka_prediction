...
def argsParser():
    ...
    parser.add_argument("--n_random_smiles", help="random smiles", type=int, default=0)
    ...
    parser.add_argument("--hidden_channels", help="Hidden node feature dimensionality", type=int, default=256)
    parser.add_argument("--num_timesteps", help="Number of iterative refinement steps for global readout", type=int, default=2)
    parser.add_argument("--encoder_name", help="for KANO: CMPNN (default) or MPNN", type=str, default='CMPNN')
    parser.add_argument("--ensemble_size", help="for KANO: Number of models in ensemble", type=int, default=1)
    parser.add_argument('--hidden_size', help='for KANO: Dimensionality of hidden layers in MPN', type=int, default=300)
    parser.add_argument('--bias', help='for KANO: Whether to add bias to linear layers', action='store_true', default=False)
    parser.add_argument('--depth', help='for KANO: Number of message passing steps', type=int, default=4)
    parser.add_argument('--activation', help='for KANO: Activation function', type=str, default='ReLU',
                        choices=['ReLU', 'LeakyReLU', 'PReLU', 'tanh', 'SELU', 'ELU', 'GELU'])
    parser.add_argument('--undirected', help='for KANO: Undirected edges (always sum the two relevant bond vectors)', action='store_true', default=False)
    parser.add_argument('--ffn_hidden_size', help='for KANO: Hidden dim for higher-capacity FFN (defaults to hidden_size)', type=int, default=2)
    parser.add_argument('--ffn_num_layers', help='for KANO: Number of layers in FFN after MPN encoding', type=int, default=2)
    parser.add_argument('--atom_messages', help='for KANO: Use messages on atoms instead of messages on bonds', action='store_true', default=False)
    parser.add_argument("--dump_path", help="for KANO: Experiment dump path", default="dumped", type=str)
    parser.add_argument("--exp_name", help="for KANO: Experiment name", default="KANO_test", type=str)
    parser.add_argument("--exp_id", help="for KANO: Experiment ID", default="1", type=str)
    parser.add_argument("--step", help="for KANO: pretraining or finetuning", default="functional_prompt", type=str,
    ...
    parser.add_argument("--csv_path", help="csv path", type=str, default='../Datasets/trbl.csv')
    parser.add_argument("--verbose", help="amount of data output", type=int, default=0)
    parser.add_argument("--print_models", help="amount of data output", type=str2bool, default=True)
    ...
    parser.add_argument("--train_pickled", help="train pickled", type=str, default='..\\Datasets\\pickled_data\\train_pickled.pkl')
    parser.add_argument("--test_pickled", help="test pickled", type=str, default='..\\Datasets\\pickled_data\\test_pickled.pkl')
    parser.add_argument("--carbons_included", help="including carbon ionization center", type=str2bool, default=False)
    parser.add_argument("--model_dir", help="directory where model is", type=str, default='../Model/')
    parser.add_argument("--model_name", help="model file name", type=str, default='train_AAc-1_best.pth')
    ...
    if args.mode == 'train' or args.mode == 'hyperopt':
        ...
        print('| --GATv2Conv_Or_Other (GATv2Conv, TransformerConv, AttentiveFP, KANO): %-52s |' % args.GATv2Conv_Or_Other)
        ...
        print('| --num_timesteps (for attentiveFP):                                 %-55.2f |' % args.num_timesteps)
        print('| --hidden_channels (for attentiveFP):                               %-55.2f |' % args.hidden_channels)
        print('| --encoder_name (KANO: CMPNN (default) or MPNN)                     %-55s |' % args.encoder_name)
        print('| --ensemble_size (KANO: Number of models in ensemble)               %-55.0f |' % args.ensemble_size)
        print('| --hidden_size (KANO: Dimensionality of hidden layers in MPN)       %-55.0f |' % args.hidden_size)
        print('| --bias (KANO: Whether to add bias to linear layers)                %-55s |' % args.bias)
        print('| --depth (KANO: Number of message passing steps)                    %-55.0f |' % args.depth)
        print('| --activation (KANO: Activation function)                           %-55s |' % args.activation)
        print('| --undirected (KANO: Undirected edges)                              %-55s |' % args.undirected)
        print('| --ffn_hidden_size (KANO: Hidden dim for higher-capacity FFN (defaults to hidden_size)) %-35.0f |' % args.ffn_hidden_size)
        print('| --ffn_num_layers (KANO: Number of layers in FFN after MPN encoding)%-55.0f |' % args.ffn_num_layers)
        print('| --atom_messages (KANO: Use messages on atoms instead of messages on bonds)   %-45s |' % args.atom_messages)
        print('| --dump_path (KANO: Experiment dump path)                           %-55s |' % args.dump_path)
        print('| --exp_name (KANO: Experiment name - required)                      %-55s |' % args.exp_name)
        print('| --exp_id (KANO: Experiment ID - required)                          %-55s |' % args.exp_id)
        print('| --step (KANO: pretraining or finetuning)                           %-55s |' % args.step)
    ...
    if args.mode == 'test' or args.mode == 'test_with_IC' or args.mode == 'infer' or args.mode == 'pH':
